...
import com.example.projectplan.ml.TransferModel;
    ...
    private void processData(List<Double> redData, int frameRate) {
        ...
        Thread mThread = new Thread() {
            ...
            public void run() {
                ...
                float[] normalizedSignalArray = obj.toJava(float[].class);

// Create a ByteBuffer from the Java array
                ByteBuffer byteBuffer = ByteBuffer.allocateDirect(normalizedSignalArray.length * Float.SIZE / 8)
                        .order(ByteOrder.nativeOrder());
                byteBuffer.asFloatBuffer().put(normalizedSignalArray);
                String sbp = "";
                String dbp = "";

                try {
                    TransferModel model = TransferModel.newInstance(MainActivity2.this);

                    // Creates inputs for reference.
                    TensorBuffer inputFeature0 = TensorBuffer.createFixedSize(new int[]{1, 875, 1}, DataType.FLOAT32);
                    inputFeature0.loadBuffer(byteBuffer);

                    // Runs model inference and gets result.
                    TransferModel.Outputs outputs = model.process(inputFeature0);
                    TensorBuffer outputFeature0 = outputs.getOutputFeature0AsTensorBuffer();
                    TensorBuffer outputFeature1 = outputs.getOutputFeature1AsTensorBuffer();

                    // Releases model resources if no longer used.
                    model.close();
                    float[] outputData1 = outputFeature0.getFloatArray();
                    float[] outputData0 = outputFeature1.getFloatArray();

                    // Convert numeric outputs to strings
                    StringBuilder stringBuilder0 = new StringBuilder();
                    for (float value : outputData0) {
                        stringBuilder0.append(String.valueOf(value)).append(" ");
                    }
                    String outputString0 = stringBuilder0.toString().trim();

                    StringBuilder stringBuilder1 = new StringBuilder();
                    for (float value : outputData1) {
                        stringBuilder1.append(String.valueOf(value)).append(" ");
                    }
                    String outputString1 = stringBuilder1.toString().trim();
...
//                        String[] obtained_result = obj.toString().split(" ");
                        hrView.setText("SBP: " + finalSbp + " mmHG");
                        hrvView.setText("DBP: " + finalDbp + " mmHG");
                        vlf.setText("-------");
                        lf.setText("-------");
                        hf.setText("");
                        ratio.setText("");
